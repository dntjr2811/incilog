...
**① 登録するとき** — アラームを「意味のベクトル」に変換して保存

```mermaid
flowchart LR
    A["📝 アラーム本文<br/>「ディスク残量が逼迫…」"]
    B["🧠 AI埋め込みモデル<br/>multilingual-e5<br/>(ONNX / ローカル実行)"]
    C["🔢 ベクトル（1024次元）<br/>[0.12, -0.84, 0.33, …]"]
    D[("🗄️ pgvector (DB)<br/>ベクトルを保存")]
    A --> B -->|意味を数値化| C --> D
```

**② 検索するとき** — 選んだアラームに「意味が近い」過去ログを探す

```mermaid
flowchart LR
    E["🖱️ 選んだアラーム"]
    F["🧠 AIでベクトル化"]
    G[("🗄️ pgvector<br/>「近いベクトル」を問い合わせ")]
    H["✅ 似たアラームを返す<br/>コサイン類似度が高い順<br/>（同一ホスト内・対応済みに絞込）"]
    E --> F --> G --> H
```
...
flowchart TD
    User["👤 ブラウザ"]
    NGINX["🌐 nginx<br/>リバースプロキシ（入口・唯一の公開ポート 8080）"]
    FE["🖥️ frontend<br/>React 19（画面・操作UI）"]
    BE["⚙️ backend<br/>FastAPI（API・業務ロジック）"]
    AI["🧠 AI埋め込みモデル<br/>multilingual-e5（ONNX・ローカル実行）<br/>※ backendに同梱／./models をマウント"]
    DB[("🗄️ db<br/>PostgreSQL 16 + pgvector<br/>（ログ保存＋ベクトル検索）")]
    VOL[("💾 pgdata<br/>データ永続化")]
